use pedigree::{InferenceGraphBuilder, Origin, Pedigree};
...
fn test_backward_iterator() {
    ...
    let inf_graph = vec![
        ...
        .fold(Pedigree::new(), |ancs, (id, origin)| {
            ancs.insert(id, origin)
        })
    ...
    assert_eq!(
        ...
        Some((
            ...
            vec![&vec![d_id3.clone()].into_iter().collect()]
                .into_iter()
                .collect()
        ))
    );
    ...
    assert_eq!(
        ...
        Some((
            vec![(&a_id0, None)],
            vec![
                &vec![a_id1.clone(), d_id1.clone()].into_iter().collect(),
                &vec![a_id2.clone(), d_id2.clone()].into_iter().collect(),
                &vec![d_id3.clone()].into_iter().collect(),
            ]
        ))
...
fn test_extract_inference_graph() {
    ...
    assert_eq!(
        actual_inference_graph.entries_by_generation, expected_inference_graph.entries_by_generation,
        "Checking entries_by_generation"
    );
    assert_eq!(
        actual_inference_graph.entries_to_generation, expected_inference_graph.entries_to_generation,
        "Checking entries_to_generation"
    );
    assert_eq!(
        actual_inference_graph.leaves, expected_inference_graph.leaves,
        "Checking leaves"
    );
    assert_eq!(
        actual_inference_graph.root, expected_inference_graph.root,
        "Checking root"
    );
    assert_eq!(
        actual_inference_graph.pedigree, expected_inference_graph.pedigree,
        "Checking pedigree"
    );
    assert_eq!(
        actual_inference_graph, expected_inference_graph,
        "Checking all"
    );
}
